...
<div class="row">
    <div class="col-12">
        <form action="{{ route('product.update', $product->id) }}" method="POST" class="card" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-12 col-md-6">
                        <div class="row gy-3">
                            <div class="col-12">
                                ...
                                <div class="row" id="images-preview">
                                    @foreach ($product->images as $image)
                                        <div class="col-6 col-md-4 col-xxl-3 mb-3 position-relative" data-id="{{ $image->id }}">
                                            <input type="hidden" name="ids[]" value="{{ $image->id }}">
                                            <img src="{{ asset('storage/shop/products/' . $image->image) }}" height="150" class="w-100 object-fit-contain img-border" alt="Preview Image">
                                            <button type="button" class="btn btn-danger btn-icon-circle btn-icon-circle-sm position-absolute top-0 end-0 me-2 delete-image" data-bs-toggle="modal" data-bs-target="#modal-delete" data-route="{{ route('product.image.delete', [$product->id, $image->id]) }}">
                                                <i class="ti ti-x"></i>
                                            </button>
                                        </div>
                                    @endforeach
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control form-control-lg" id="product-name" name="product_name" value="{{ @old('name') ?? $product->name }}" required>
                            </div>
                            <div class="col-md-12">
                                <label for="product-code" class="form-label">Product Code</label>
                                <input type="text" class="form-control" id="product-code" name="product_code" value="{{ @old('product_code') ?? $product->product_code }}">
                            </div>
                            <div class="col-md-6">
                                <label for="color" class="form-label">Color <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="color" name="color" value="{{ @old('color') ?? $product->color }}" autocomplete="off" required>
                            </div>
                            <div class="col-md-6">
                                <label for="price" class="form-label">Price <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="price" name="price" value="{{ @old('price') ??'Rp. ' . number_format($product->price, 0, ',', '.') }}" autocomplete="off" required>
                            </div>
                            <div class="col-12 col-md-6">
                                ...
                                <select type="text" class="form-select" id="main-category" name="main_category" onchange="getProductCategory(this.value)" required>
                                    ...
                                    @foreach ($main_categories as $main_category)
                                        <option value="{{ $main_category->id }}" {{ $main_category->id == $product->product_category->main_category_id ? 'selected' : '' }}>{{ $main_category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 col-md-6">
                            ...
                            <div class="col-12">
                                ...
                                <div id="quill-description" style="height: auto">{!! old('description') ?? $product->description !!}</div>
                            </div>
                            <div class="col-12 image-preview d-flex flex-column">
                                ...
                                <img src="{{ asset('storage/shop/products/size_guides/' . $product->size_guide) }}" alt="" height="300" onclick="triggerFile('size-guide')" class="object-fit-contain img-border mb-2">
                            ...
                            <div class="col-md-4">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="preorder" name="preorder" onchange="let pd = this.checked ? document.getElementById('preorder-days').value : ''; document.getElementById('preorder-days').value = pd;" data-bs-toggle="collapse" data-bs-target="#preorder-collapse" {{ $product->is_preorder == 1 ? 'checked' : '' }}>
                                    <label for="preorder" class="form-label">Preorder</label>
                                </div>
                                <div id="preorder-collapse" class="collapse {{ $product->is_preorder == 1 ? 'show' : '' }} input-group">
                                    <input type="text" class="form-control" id="preorder-days" name="preorder_days" value="{{ @old('preorder_days') ?? $product->preorder_days }}" placeholder="Preorder Days" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" autocomplete="off">
                                ...
                                <div id="size-container">
                                    ...
                                    @else
                                        ...
                                        <div class="input-group mb-3">
                                            ...
                                            <span class="input-group-text">Size</span>
                                            <input type="text" class="form-control" id="size-{{ $loop->iteration }}" name="size[]" value="{{ $size->size }}" required autocomplete="off">
                                            <span class="input-group-text">Stock</span>
                                            <input type="text" class="form-control" id="stock-{{ $loop->iteration }}" name="stock[]" value="{{ $size->stock }}" required autocomplete="off">
                                            <button class="btn btn-outline-danger delete-image" data-bs-toggle="modal" data-bs-target="#modal-delete" data-route="{{ route('product.size.delete', [$product->id, $size->id]) }}" type="button"><i class="ti ti-trash"></i></button>
                                        </div>
                                        @endforeach
                                    @endif
                                </div>
